...
public class EntityManagerImpl extends LoggingObject implements EntityManager {
    ...
    @Override public boolean saveDbIndexes() {
        try {
            ...
            List<String> databaseEntities = getEntityNamesInDatabase();
            int entitiesSize = entities.size();
            for(int i=entitiesSize-1;i>=0;i--) {
               if(!databaseEntities.contains(entities.get(i).get("info").get("title").asText().toLowerCase()+ENTITY_FILE_EXTENSION)) {
    ...
    private List<String> getEntityNamesInDatabase() {
        DatabaseClient stagingClient = hubConfig.newStagingClient();
        QueryManager queryMgr = stagingClient.newQueryManager();
        StructuredQueryBuilder entitiesBuilder = new StructuredQueryBuilder();
        StructuredQueryDefinition query = entitiesBuilder.and(entitiesBuilder.collection(ENTITY_COLLECTION_STAGING));
        StringHandle requestHandler = new StringHandle();
        requestHandler.setFormat(Format.JSON);
        String entitiesListString = queryMgr.search(query, requestHandler).get();

        List<String> uris = new ArrayList<>();
        try {
            JSONArray resultsArray = (JSONArray) new JSONObject(entitiesListString).get("results");
            JSONObject entityObject = null;
            for(int i=0;i<resultsArray.length();i++) {
                entityObject = new JSONObject(resultsArray.get(i).toString());
                String[] uri = entityObject.get("uri").toString().split("/");
                uris.add(uri[uri.length-1].toLowerCase());
            }
