...
const JobResultsTableView = (props) => {
  ...
  const [colHeaders, setColHeaders] = useState(allColumnHeaders);
  ...
  const onApply = () => {
    const checkedColumns = CONFIGURABLE_HEADERS.filter(columnHeader => checkedEntityColumns[columnHeader.dataIndex]);
    const filteredColumns =  [...MANDATORY_HEADERS, ...checkedColumns];
    setColHeaders(filteredColumns);
    setPopoverVisibility(false);
  };
        ...
        <div className={styles.content}>
            <Menu>
                {Object.keys(checkedEntityColumns).map(entLabel => (
                    <Menu.Item key={entLabel} className={styles.DropdownMenuItem}><MLCheckbox
